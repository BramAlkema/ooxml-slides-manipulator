...
class SuperThemeEditor {
  ...
  analyzeSuperTheme(superThemeBlob) {
    ...
    try {
      // Extract SuperTheme files using OOXMLCore abstraction
      const ooxml = new OOXMLCore(superThemeBlob);
      const extractedFiles = ooxml.extractFiles();
  ...
  createSuperTheme(superThemeDefinition) {
    ...
    try {
      ...
      // Package into .thmx file using OOXMLCore
      const ooxml = new OOXMLCore();
      ooxml.replaceFiles(files);
      const superThemeBlob = ooxml.generateBlob();
